...
/**
 * Created by Shelley on 7/22/2017.
 *
 * This class is used to edit a Book that is already in the database.
 */
...
public class EditBook extends ActionBarActivity {
    ...
    private int isOwned = 0; // by default
    private int nIsOwned; // new isOwned status
    private int rating;
    ...
    @SuppressLint("SimpleDateFormat") protected void onCreate(Bundle savedInstanceState) {
        ...
        // generate the Book object from the database
    ...
    public void updateBook(View view) {
        ...
        if(!ISBN.equals(updateISBN.getText().toString())){
            book.setIsbn(updateISBN.getText().toString());
        }
        if(!author.equals(updateAuthor.getText().toString())){
            book.setAuthor(updateAuthor.getText().toString());
        }
        if(!title.equals(updateTitle.getText().toString())){
            book.setTitle(updateTitle.getText().toString());
        }

        // update date read
        if (!dRead.equals(updateDate.getText().toString())) {
            book.setDateRead(updateDate.getText().toString());
        }
        // update comments
        if (!comments.equals(updateComments.getText().toString())) {
            book.setComments(updateComments.getText().toString());
        }
